const regaliasSolicitadas = (name, currencyText, accountType, accountValue, paymentMethodText, currencyRate, totalAskedCurrency, idTransaction) => {
  const emailHtml = `<p>¡Hola <em><strong>${name}</strong></em>! ¿Cómo estás?</p><p>Este email es para confirmar que recibimos tu solicitud de cobro de regalías en <em>${currencyText}</em> a través de <em>${paymentMethodText}</em> el día de hoy. </p>
...
module.exports = { regaliasSolicitadas }